...
class Attack:
    ...
    def attack_step(self, results):
        for p in self.pipeline[0:2]:
            results = p(results)
        for hook in self.attack_step_hooks:
            results = hook(results)
        return results

    def attack_epoch(self,adv_img,data,idx):
        ...
        '''for idx, data in enumerate(mmcv.track_iter_progress(self.dataloader)):
            img_metas_data = data['img_metas'][0].data#!!!!!
            filename = img_metas_data[0][0]['ori_filename']#!!!!!
        ...
        results['data_adv_imgs'] = adv_img#!!!!!
        results = self.attack_step(results)
        losses += results['loss_combined']
